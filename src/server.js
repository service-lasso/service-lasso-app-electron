...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Service Lasso App Electron</title>
  ...
  <body>
    <main>
      <section class="panel">
        <span class="eyebrow">Electron Shell</span>
        <h1>Electron-ready shell for Service Lasso</h1>
        <p>
          This bounded host keeps the runtime and admin wiring executable today, while the future native Electron wrapper simply needs to point its window at this local shell and the tracked service inventory.
        </p>
        <div class="card">
        ...
        <div class="card">
          <span class="label">Electron config path</span>
          <code>${config.electronConfigPath}</code>
        </div>
        <div class="links">
          <a class="link" href="/api/host-status">
            <strong>Host status JSON</strong>
            <span>Inspect shell wiring, runtime roots, and next-step Electron metadata.</span>
          </a>
...
export function createHostStatus(config) {
  return {
    app: "@service-lasso/service-lasso-app-electron",
    ...
    electronConfigPath: config.electronConfigPath,
    notes: [
      "Desktop shell is served at /.",
      ...
      "The next native Electron wrapper should open this local host URL inside the desktop shell.",
    ],
  };
}

export function createElectronHostServer(config) {
