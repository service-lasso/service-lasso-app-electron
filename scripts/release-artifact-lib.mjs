    ...
    `${JSON.stringify(
      {
        id: "echo-service",
        name: "Echo Service",
        description: "Verification harness service for the electron starter artifact.",
        version: "0.0.0",
        enabled: true,
        executable: "go",
        args: ["run", "."],
...
export async function verifyStagedArtifact({ repoRoot, artifactRoot, archivePath, version } = {}) {
  ...
  const result = await smokeStarterHost(stagedRoot, {
    ...
    SERVICE_LASSO_APP_ELECTRON_ADMIN_DIST_ROOT: fixtures.adminDistRoot,
    SERVICE_LASSO_APP_ELECTRON_ECHO_SERVICE_REPO_ROOT: fixtures.echoServiceRoot,
