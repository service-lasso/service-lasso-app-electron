...
import { resolveElectronConfig, validateElectronConfig } from "../src/config.js";
import { createHostStatus, createElectronHostServer } from "../src/server.js";

async function createFixtureRoots() {
  const root = await mkdtemp(path.join(tmpdir(), "service-lasso-app-electron-"));
  const siblingRoot = path.join(root, "siblings");
  const adminDistRoot = path.join(siblingRoot, "lasso-@serviceadmin", "dist");
  const sourceServicesRoot = path.join(root, "service-lasso-app-electron", "services");
...
test("electron config resolves deterministic sibling repo paths", async () => {
  const fixture = await createFixtureRoots();

  try {
    const config = resolveElectronConfig({
      repoRoot: path.join(fixture.root, "service-lasso-app-electron"),
    ...
    assert.match(config.electronConfigPath, /src-electron[\\/]electron\.config\.json$/);

    await assert.doesNotReject(() => validateElectronConfig(config));
  } finally {
    await rm(fixture.root, { recursive: true, force: true });
  }
});

test("desktop host serves shell, host status, and mounted admin assets", async () => {
  const fixture = await createFixtureRoots();

  try {
    const config = await validateElectronConfig(
    ...
    assert.equal(status.app, "@service-lasso/service-lasso-app-electron");
    assert.equal(status.sourceServicesRoot, fixture.sourceServicesRoot);

    const server = createElectronHostServer(config);
    server.listen(0, "127.0.0.1");
    await once(server, "listening");
    ...
    try {
      ...
      assert.match(shellHtml, /Electron-ready shell for Service Lasso/);
...
test("desktop host proxies runtime services for the host-owned widget", async () => {
  const fixture = await createFixtureRoots();
